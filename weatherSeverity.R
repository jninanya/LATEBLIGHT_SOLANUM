weatherSeverity<-function (weather, severity, dates, EmergDate, EndEpidDate, NoReadingsH, RHthreshold) 
{
  ...
  for (i in 1:nr) {
    ...
    if (Wfile[i, 2] != "") 
      Wfile[i, 6] <- as.numeric(hours)
    if (!is.na(Wfile[i, 4])) {
      if (Wfile[i, 4] > 100) 
        Wfile[i, 4] <- 100
      if (Wfile[i, 4] > RHthreshold) 
        Wfile[i, 7] <- (1/NoReadingsH)
    }
    if (!is.na(Wfile[i, 7])) {
      if (Wfile[i, 7] == (1/NoReadingsH)) 
        Wfile[i, 8] <- Wfile[i, 3]
    }
  }
